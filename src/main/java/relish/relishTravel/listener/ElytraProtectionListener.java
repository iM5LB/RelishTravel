...
public class ElytraProtectionListener implements Listener {
    ...
    public void onInventoryClick(InventoryClickEvent event) {
        ...
        // If the player is removing a real (non-virtual) elytra from the chest slot during an active
        // launch, end the launch cleanly on the next tick so the state doesn't go stale.
    ...
    public void onPlayerSwapHandItems(PlayerSwapHandItemsEvent event) {
        ...
        ItemStack chestplate = player.getInventory().getChestplate();
        if (chestplate != null && chestplate.getType() == Material.ELYTRA && isVirtualElytra(chestplate)) {
            // Virtual elytra: always block the swap key.
            event.setCancelled(true);
            return;
        }
        
        // Real elytra case: if the chestplate slot no longer holds an elytra (player already moved it
        // out to a chest), the launch state is stale. End the launch cleanly instead of letting
        // repeated F-key presses operate on inconsistent inventory state and produce duplicates.
